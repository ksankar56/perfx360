...
function execute(req, cb) {
    console.info("req.body.testId = " , req.body.testId);
    async.waterfall([
...
function _saveTestObject(req, callback) {
    ...
    testServiceImpl.getTestObject(params.testId, function (err, tests) {
        ...
        if (!_.isEmpty(tests)) {
            ...
            params.test = tests[0];
            params.testParameter = req.body;

            testServiceImpl.updateObject(req.body, function(err, test) {
                console.info('test updated');
...
function _saveTestExecutionObject(params, callback) {
    ...
    testExecutionJson.executedComponents = params.test.components;
